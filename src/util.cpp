...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "bcash" is a composite category enabling all BCash-related debug output
            if(ptrCategory->count(string("bcash"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BCashCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BCashCore
    // Mac: ~/Library/Application Support/BCashCore
    // Unix: ~/.bcashcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BCashCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BCashCore";
#else
    // Unix
    return pathRet / ".bcashcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bcash.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bcash.conf
